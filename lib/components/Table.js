...
function makeMergedRender(rowKey, render, update, del, requestConfig, user) {
  if (!render) {
    return null;
  }
...
var Table = function Table(props) {
  ...
  var defaultSyncToUrl = (0, _react.useCallback)(function (values, type) {
    if (type === 'get') {
      var newValues = _objectSpread({}, values);

      (0, _lodash.forEach)((0, _lodash.keys)(newValues), function (key) {
        var _column$valueEnum$key;

        var column = (0, _lodash.find)(newColumns, function (c) {
          return c.dataIndex === key;
        });

        if ((0, _lodash.isMap)(column === null || column === void 0 ? void 0 : column.valueEnum) && (0, _lodash.isNumber)((_column$valueEnum$key = column.valueEnum.keys()) === null || _column$valueEnum$key === void 0 ? void 0 : _column$valueEnum$key[0])) {
          newValues[key] = (0, _lodash.toNumber)(newValues[key]);
        }
      });
      return newValues;
    }
