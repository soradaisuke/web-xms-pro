...
import ProTable, { DragSortTable, ProTableProps, DragTableProps } from '@ant-design/pro-table';
import { ParamsType } from '@ant-design/pro-provider';
import { TransferItem } from 'antd/lib/transfer';

export type TableTransferItem = Omit<TransferItem, 'key'> & Required<Pick<TransferItem, 'key'>>;

export type TableTransferProps<T extends TableTransferItem = TableTransferItem, U = ParamsType> = Omit<TransferProps<T>, 'dataSource'> & Partial<Pick<TransferProps<T>, 'dataSource'>> & {
...
const TableTransfer = function<T extends TableTransferItem = TableTransferItem, U = ParamsType>(props: PropsWithChildren<TableTransferProps<T, U>>) {
  const { preserve, tableProps, leftTableProps, rightTableProps, dataSource, targetKeys, onChange, initialTargetDataSource, initialTargetKeys } = props;

  const leftProps = useMemo(() => merge({}, defaultTableProps, tableProps, leftTableProps), [tableProps, leftTableProps]);
  const rightProps = useMemo(() => merge({}, defaultTableProps, tableProps, rightTableProps), [tableProps, rightTableProps]);
  ...
  const onTransferChange = useCallback((nextTargetKeys, direction?, moveKeys?) => {
  ...
  const getTableDataSource = useCallback((direction, filteredItems) => {
    if (direction === 'left') {
    ...
    return filteredItems;
  }, [leftTableProps?.request, preserve, transferDataSource, transferTargetKeys, totalDataSource]);
  ...
  return (
    <Transfer<T> {...props} dataSource={transferDataSource} targetKeys={transferTargetKeys} onChange={onTransferChange}>
      {renderChildren}
    </Transfer>
  );
};
