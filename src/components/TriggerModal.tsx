...
const TriggerModal: React.FC<TriggerModalProps> = function(props) {
  ...
  const onMergedOk = useCallback(async (e) => {
    await onOk?.(e);
    setIsModalVisible(false);
  }, [onOk]);

  const onMergedCancel = useCallback(async (e) => {
    await onCancel?.(e);
    setIsModalVisible(false);
  }, [onCancel]);
  ...
  return (
    <>
      <Modal {...modalProps} visible={isModalVisible} onOk={onMergedOk} onCancel={onMergedCancel} />
      {
